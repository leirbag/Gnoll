...
namespace Gnoll
{
	namespace Input 
	{
		...
		MouseButtonEventsTranslator::MouseButtonEventsTranslator():
		...
		void MouseButtonEventsTranslator::handle ( shared_ptr<Message> message )
		{
			/**
			 * Get the key code
			 */
		...
		void MouseButtonEventsTranslator::trigger ( shared_ptr<Message> _msg )
		{
			Messages::MessageType actionEventType(ACTION_EVENT_TYPE);
			TimeModule* timeModule = TimeModule::getInstancePtr();
			...
			for( map<string, unsigned long int>::iterator it = m_durationButtonPressed.begin(); it != m_durationButtonPressed.end(); it++)
			{
				...
				if (timePressed > 0)
				{
					...
					shared_ptr<List> actionList = m_mouseButtonEventTranslationMap->getAttribute<List>( it->first );
